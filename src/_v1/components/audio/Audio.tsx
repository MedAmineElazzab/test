...
export default function AudioPlayer({
  ...
  return (
    ...
    >
      ...
      ></div>
      <div
        className={`absolute z-[8] w-[280px] transition-all ${
          isPlaying ? "opacity-40" : "opacity-0"
        } pointer-events-none`}
      >
        <LottieAnimation loop animationData={waves} />
      </div>
      <div
        className={`absolute audio-clip-btn z-[20] ${
          isPlaying ? "playing" : "off"
        } top-1/2 left-1/2 block -translate-x-1/2 border-[4px] border-white rounded-full -translate-y-1/2 profile w-[45px] h-[45px]`}
      >
        <img
